...
case class Client(id: Int,
  ...
  override def toString: String = {
    val gender = if (sex) "male" else "female"
    s"User $id: $name, $gender, born $birthdate, address $address"
  }
...
object Client {
  ...
  def fetchAll: Query0[Client] =
    sql"""SELECT * from KHINKA."Users"""".query[Client]
}
